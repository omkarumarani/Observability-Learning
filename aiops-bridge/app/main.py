...
async def _create_xyops_ticket(
    ...
    """
    Create an AI-enriched incident ticket in xyOps, then open an
    approval-gate ticket if the severity warrants human sign-off.

    Steps:
      1. Fetch recent Loki logs + Prometheus metrics for the service
      2. Call Claude to generate RCA, Ansible playbook, PR description
      3. Create the incident ticket (enriched body)
      4. If REQUIRE_APPROVAL=true → create approval-gate ticket
    """
    # ── 1. Gather context (parallel) ────────────────────────
    if _http:
        logs, metrics = await __import__("asyncio").gather(
            fetch_loki_logs(service_name, _http),
            fetch_prometheus_context(service_name, _http),
        )
    else:
        logs, metrics = "", {}
    ...
    result = await _xyops_post("/api/app/create_ticket/v1", payload)
    if result.get("error"):
        logger.warning(
            "Failed to create xyOps ticket  alert=%s  service=%s  error=%s",
            alert_name,
            service_name,
            result.get("detail", result.get("error")),
        )
        return {"ticket_id": "", "xyops_response": result}

    ticket_id = result.get("ticket", {}).get("id", "")
    logger.info(
        "Created xyOps ticket %s  alert=%s  service=%s  severity=%s  ai=%s  trace=%s",
        ticket_id,
        alert_name,
        service_name,
        severity,
    ...
    if (
        ...
        logger.info(
            "Approval gate created  approval_id=%s  ticket=%s",
            approval_id,
            approval_ticket_id,
        )
...
async def _resolve_xyops_tickets(
    ...
    """
    Find all open xyOps tickets for this alert+service and close them.

    Uses xyOps tag search to find matching open tickets then updates
    each one to status=closed with a resolution note.
    """
    # Search for open AIOPS tickets whose subject contains both alert and service names.
    # The subject is set to "[AIOPS] {alert_name} on {service_name} ...", so both words
    # appear and can be found by Unbase full-text search.
