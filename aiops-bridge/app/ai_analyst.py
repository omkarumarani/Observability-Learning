...
───────────────────────────────────────────────────────────────
AI Analyst — Claude-powered RCA, Ansible playbook generation,
and GitHub PR description writer.

Pipeline per alert:
  1. Fetch recent logs from Loki (last 50 lines for the service)
  2. Fetch key metrics from Prometheus (error_rate, p99, throughput)
  3. Call Claude to produce:
       a. Root Cause Analysis (narrative + structured)
       b. Ansible playbook (YAML) to remediate
       c. GitHub PR description for code/config changes
       d. Test plan for the playbook (dry-run steps)
  4. Returns enriched context dict used by main.py to build the ticket

All Loki/Prometheus calls go through the shared httpx client so they
appear as OTel child spans in Tempo.
...
AI_ENABLED: bool = bool(CLAUDE_API_KEY)

# Claude API endpoint (Anthropic Messages API)
...
async def generate_ai_analysis(
    ...
    """
    if not AI_ENABLED:
        logger.info("AI analysis skipped (CLAUDE_API_KEY not set)")
        return {}
