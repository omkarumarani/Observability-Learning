...
class ApprovalRequest:
    ...
    # xyOps approval-gate ticket ID (so we can update it on decision)
...
async def request_approval(
    ...
    logger.info(
        "Approval gate ticket %s created  approval_id=%s  incident=%s",
...
async def _execute_playbook(
    ...
    # Also append result to the original incident ticket
    await _update_incident_ticket(req=req, comment=comment, xyops_post=xyops_post)
...
async def _update_approval_ticket(
    ...
    await xyops_post(
        ...
        {"id": req.approval_ticket_id, "type": "comment", "data": comment},
    )
...
async def _update_incident_ticket(
    ...
    await xyops_post(
        "/api/app/add_ticket_change/v1",
        {
            "id": req.incident_ticket_id,
            "type": "comment",
            "data": f"## Remediation Update\n\nSee approval ticket `{req.approval_ticket_id}`\n\n{comment}",
        },
    )
